...
from epymorph.data_type import CentroidDType, SimArray
...
from epymorph.initializer import IndexedLocations, NoInfection, SingleLocation
...
class Init(IndexedLocations):
    ...
    def __init__(self, location: int, seed_size: int):
        super().__init__(
...
def test_rume_create_multistrata_1():
    ...
    rume = MultiStrataRUME.build(
        strata=[
            GPM(
                ...
                ipm=Sir(),
                mm=No(),
                init=Init(location=0, seed_size=100),
            ),
            GPM(
                name="bbb",
                ipm=Sir(),
    ...
    assert rume.requirements == {
        AN("gpm:aaa::ipm::beta"): AD("beta", float, Shapes.TxN),
        AN("gpm:aaa::ipm::gamma"): AD("gamma", float, Shapes.TxN),
        AN("gpm:bbb::ipm::beta"): AD("beta", float, Shapes.TxN),
        AN("gpm:bbb::ipm::gamma"): AD("gamma", float, Shapes.TxN),
        AN("gpm:aaa::init::population"): AD(
            "population", int, Shapes.N, comment="The population at each geo node."
        ),
        AN("gpm:bbb::init::population"): AD(
            "population", int, Shapes.N, comment="The population at each geo node."
        ),
    }
...
def test_rume_create_multistrata_2():
    ...
    assert rume.requirements == {
        ...
        AN("gpm:aaa::mm::commuter_proportion"): AD(
            "commuter_proportion",
            float,
            Shapes.Scalar,
            default_value=0.1,
...
def test_requirements_tree_precedence():
    # RUME parameters take precedence over GPM parameter values
    params = _default_params()
    params["gpm:bbb::mm::phi"] = 99.0
    ...
    phi_bbb = _assert_has_node(tree, "gpm:bbb::mm::phi")
    assert phi_bbb.value == 99.0
    assert phi_bbb.resolution == ParameterValue(
        cacheable=True,
        pattern=NamePattern.parse("gpm:bbb::mm::phi"),
    )
