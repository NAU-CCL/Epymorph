"""
`CustomIPMExecutor` is part of the internal workings of `NBSimulator`, implementing the
...
class CustomIPMExecutor:
    ...
    def _events(self, tick: Tick, node: int, effective_pop: SimArray) -> SimArray:
        ...
        R = self._rume.params[NamePattern.of('R')]
            ...
            try:
                match t:
                    case CompiledEdge(rate_lambda):
                        ...
                        occur[index] = self._rng.negative_binomial(p = R/(rate * tick.tau + R), n = R)
                    case CompiledFork(size, rate_lambda, prob_lambda):
                        ...
                        occur[index : (index + size)] = self._rng.multinomial(
                            n=self._rng.negative_binomial(p = R/(rate * tick.tau + R), n = R),
                            pvals=prob,
                        )
                index += t.size
