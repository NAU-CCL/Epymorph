...
class SelectionInitializer(Initializer, ABC):
    ...
    def apply(self, ctx: SimContext, out: list[Compartments]) -> None:
        size = _get_param(ctx, "infection_seed_size", self.infection_seed_size)
        ...
        if available < size:
            raise Exception(
                f"{self.__class__.__name__} was asked to infect {size} people but only had {available} available")
    ...
    @abstractmethod
    def select(self, ctx: SimContext) -> NDIndices:
        """Given the context, return the indices of which nodes to infect."""
        pass


class SingleLocInitializer(SelectionInitializer):
...
class TopLocsInitializer(SelectionInitializer):
    ...
    def select(self, ctx: SimContext) -> NDIndices:
        ...
        attribute = _get_param(ctx, "infection_attribute",
                               self.infection_attribute)
        if attribute not in ctx.geo:
            raise Exception(
                f"{self.__class__.__name__} attempted to pick nodes by geo[{attribute}], but that attribute is missing.")
...
DEFAULT_INITIALIZER = SingleLocInitializer()
