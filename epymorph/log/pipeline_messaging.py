...
from epymorph.event import ADRIOProgress, EventBus, OnPipelineProgress, OnPipelineStart
from epymorph.log.messaging import EPYMORPH_LIVE_MESSAGING
...
def pipeline_messaging(
    *,
    ...
    def on_pipeline_start(e: OnPipelineStart) -> None:
        ...
        total_progress = e.total_progress

        print(f"Running simulation ({e.name}):")
        print(f"• {start_date} to {end_date} ({duration_days} days)")
        print(f"• {e.rume.scope.nodes} geo nodes")
        print(f"• {e.num_realizations} realizations")
        if live:
    ...
    def on_pipeline_progress(e: OnPipelineProgress) -> None:
        # NOTE: tick updates will be skipped entirely if `live=False`
        nonlocal last_progress_length
        nonlocal total_progress
        nonlocal progress_complete
        progress_complete += e.progress
        total_process_time = perf_counter() - start_time
        average_process_time = total_process_time / progress_complete
    ...
    def on_pipeline_finish(_: None) -> None:
    ...
    with subscriptions() as subs:
        ...
        subs.subscribe(_events.on_pipeline_start, on_pipeline_start)
        if live:
            subs.subscribe(_events.on_pipeline_progress, on_pipeline_progress)
        subs.subscribe(_events.on_pipeline_finish, on_pipeline_finish)
        if adrio:
