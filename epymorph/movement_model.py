...
def _validate_movement_clause(clause_cls: type[MovementClause]) -> None:
    """Validate a Movement Clause definition."""
    name = clause_cls.__name__

    # Check predicate.
    predicate = getattr(clause_cls, "predicate", None)
    ...
    # Check leaves.
    leaves = getattr(clause_cls, "leaves", None)
    ...
    # Check returns.
    returns = getattr(clause_cls, "returns", None)
...
def _validate_movement_model(model_cls: type[MovementModel]) -> None:
    """Validate a Movement Model definition."""
    name = model_cls.__name__

    # Check tau steps.
    steps = getattr(model_cls, "steps", None)
    ...
    # Check clauses.
    clauses = getattr(model_cls, "clauses", None)
    ...
    # Make steps and clauses immutable.
    model_cls.steps = tuple(steps)
    model_cls.clauses = tuple(clauses)
