...
def map_data_by_block_group(
    scope: BlockGroupScope,
    data: NDArray,
    *,
    title: str,
    cmap: Any | None = None,
    ...
    """
    gdf_bg = us_tiger.get_block_groups_geo(year)
    gdf_bg = gdf_bg[gdf_bg["GEOID"].isin(scope.get_node_ids())]
    gdf_borders = None
...
def _map_data_by_geo(
    ...
    *,
    ...
    gdf_borders.plot(ax=ax, linewidth=0.1, edgecolor='black', color='none', alpha=0.8)
    fig.tight_layout()
    plt.show()
