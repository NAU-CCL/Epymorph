...
from epymorph.simulator.basic.forecast_basic_simulator import ForecastBasicSimulator
from epymorph.simulator.basic.forecastoutput import ForecastOutput

RUMEType = TypeVar("RUMEType", bound=RUME)


class ForecastSimulator2(Generic[RUMEType]):
    rume: RUMEType
    ...
    def run(
        ...
        # Initialize arrays to hold the aggregated data for all realizations
        aggregated_initial_values = []
        aggregated_visit_compartments = []
        aggregated_home_compartments = []
        aggregated_visit_events = []
        aggregated_home_events = []
        aggregated_params = []

        # Propagate each particle using the final mean values
        for r, particle in enumerate(particles):
            # Apply the model to each particle and collect the results
            result = self.propagate_particle(rume, particle, final_param_values, rng, r)
            # observation_results.append(result)

            # Append the results for this realization to the aggregated lists
            aggregated_initial_values.append(result.initial)
            aggregated_visit_compartments.append(result.visit_compartments)
            aggregated_home_compartments.append(result.home_compartments)
            aggregated_visit_events.append(result.visit_events)
            aggregated_home_events.append(result.home_events)
            aggregated_params.append(final_param_values)

        # Convert lists of arrays to 3D arrays (R realizations)
        # aggregated_rumes = np.stack(aggregated_rumes, axis=0)
        aggregated_initial_values = np.stack(aggregated_initial_values, axis=0)
        aggregated_visit_compartments = np.stack(aggregated_visit_compartments, axis=0)
        aggregated_home_compartments = np.stack(aggregated_home_compartments, axis=0)
        aggregated_visit_events = np.stack(aggregated_visit_events, axis=0)
        aggregated_home_events = np.stack(aggregated_home_events, axis=0)
        aggregated_params = np.stack(aggregated_params, axis=0)

        final_output = ForecastOutput(
            rume=rume,
            initial=aggregated_initial_values,  # These would also be aggregated similarly
            visit_compartments=aggregated_visit_compartments,
            visit_events=aggregated_visit_events,
            home_compartments=aggregated_home_compartments,
            home_events=aggregated_home_events,
        ...
        #     visit_compartments=result.visit_compartments,
        #     visit_events=result.visit_events,
        #     home_compartments=result.home_compartments,
        #     home_events=result.home_events,
        #     final_param_values
        # )
    ...
    def propagate_particle(self, rume, particle, parameters, rng, r):
