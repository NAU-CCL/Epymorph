...
class ForecastBasicSimulator(Generic[RUMEType]):
    """
    ...
    def __init__(self, rume: RUMEType, r: int, parameters: dict):
        self.rume = rume
        self.R = r
    ...
    def run(
        ...
        visit_events = np.zeros((self.R, S, N, E), dtype=SimDType)
        home_compartments = np.zeros((self.R, S, N, C), dtype=SimDType)
        home_events = np.zeros((self.R, S, N, E), dtype=SimDType)
