...
class ForecastOutput(
    ...
    params: NDArray[SimDType]  # dict[str, NDArray[SimDType]]
    """Dictionary of simulation parameters, with each key pointing to a numpy array
    of shape (R, ...), where R is the number of realizations."""

    # R: int
    # """Number of realizations or particles."""
    ...
    def _with_data_mode(self, data_mode: Literal["visit", "home"]) -> "ForecastOutput":
        return (
            self
    ...
    @cached_property
    def data_by_visit(self) -> "ForecastOutput":
    ...
    @cached_property
    def data_by_home(self) -> "ForecastOutput":
        """Returns an Output object that contains the same set of data, but uses
    ...
    def dataframe(self) -> pd.DataFrame:
        ...
        return pd.concat(
            (
                ...
                pd.DataFrame(
                    {
                        # "realization": np.repeat(np.arange(self.R), N * S),
