...
from epymorph.cache import (CACHE_PATH, Directory, FileError, cache_inventory,
                            cache_remove_confirmation, format_file_size)
...
def define_argparser(command_parser: _SubParsersAction):
    ...
    p = command_parser.add_parser(
        'cache',
        help="manage epymorph's file cache")
    ...
    list_command = sp.add_parser(
        "list", help="list the contents of the cache")
    ...
    remove_command.add_argument(
        "path",
        type=str,
        help="the relative path to a file or folder in the cache")
    remove_command.set_defaults(handler=lambda args: handle_remove(
        path=args.path
    ))
